...
def ChangeMaterial(materialId):
    if materialId == 1:
        ...
        prefix = "1D_Z" #ZBrushedMaterial
    elif materialId == 5:
        prefix = "1D.BW" #BWMaterial
...
class MainPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()        
        row.operator("wm.bw", icon= 'MESH_MONKEY', text= "BW")                        
        row = layout.row()        
        row = layout.row()
...
class WM_OT_BW(bpy.types.Operator):
    """Open the Add Cube Dialog box"""
    bl_label = "Set Black and white textures to daz model"
    bl_idname = "wm.bw"
...
def register():
    ...
    bpy.utils.register_class(WM_OT_BW)    
...
def unregister():
    ...
    bpy.utils.unregister_class(WM_OT_BW)    
